        ...
        try:
            ...
            if X_te.shape != Xt_te_i_j.shape:
                raise ValueError(f"Augmentation result seems invalid for "
                                 ...
                                 f"{n_random_repeats}.")
            # plot and save exemplary results for subsequent manual objective
            # checking
            fig_i_j = aug.plot_augmentation_examples(aug_obj_i_j, X_te, y_te)
...
for err_dict in err_list:
    err_dict_short = err_dict
    del err_dict_short["traceback"]
    print(err_dict_short)

# perform testing of sequential
